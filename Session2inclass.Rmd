...
1. There is a very high correlation (0.64) between "Q1_14 When buying accessories for my boat, I tend to buy the latest and greatest" and "Q1_13 When buying a boat I tend to buy the latest and greatest". This is as expected since it is likely that people who do the former are more likely to also do the latter
2. They imply potential to segment the the question to reduce the number of variables
...
**Your Answers here:**
1. The table and plot show the explanatory power of different variables vis-a-vis the other variables. The higher the eigenvalue, the more explanatory power. The table also shows the (cumulative) explaned variance as a check. The screeplot then sums it all up.
2. No the plotted line cannot be increasing. The variable with the highest eigenvalue should come first, resulting in a downward sloping plot. However, it is possible to have line with a smaller downward slope, indicating that all variables have similar explanatory power. This should be the goals of the final variables we choose to represent our sample.
3. The more concentrated the explanatory power of the variables, the better as this will result in a large reduction in the number of variables. In effect, the additional explanatory power of those variables is already represented in one component.
...
factors_selected = sum(Variance_Explained_Table_copy[,1] >= 1.5)
...
**Your Answers here:**
1. The first factors, or components, have the most explanatory power. 
2. 
a) Statistical arguments: Looking at the table with egeinvalues and variances, we can apply two statistical rules in order to choose the number of factors. First rule is that the eigenvalue should be >1 and the second rule is that cumulative explained variance should be above 50% for the last factor we choose. Both conditions are satisfied if we choose 5 factors.
Interpretation arguments
b) Interpretation arguments: In general, there is little overlap between the 5 factors and most of the bundling of variables make sense. However, when using only three factors, there is even less overlap. A drawback, however, is that we lose some of the information in the variables.  
c) From a business perspective, we now have clear segments based on the three factors: luxury, cost, adventurous

3. See answer to Q2 -> There is a clear interpretation of the segments

4. i guess that there is a difference with what is best from a statistical viewpoint, versus what is best from a interpretative/business viewpoint. Its crucial to keep checking the if the statistics make sense
